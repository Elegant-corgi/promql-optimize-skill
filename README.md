...
PowerShell 中查询带 selector 时，外层优先使用单引号，PromQL 内部双引号保持原样，不要写成反斜杠转义：
```powershell
# 正确
...
`promql-probe` 会在发起 API 请求前拦截包含 `\"` 的 query/range 查询，并给出修正建议，避免把本地 PowerShell 转义错误误诊为 Prometheus/VictoriaMetrics 的 HTTP 400。
